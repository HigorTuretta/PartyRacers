...
Oito font assets TextCore. **O peso não é uma propriedade no UI Toolkit** — cada peso é um asset próprio, e `theme.uss` tem uma classe por asset.
...
## Geração dos atlas no Unity 6

Use `Party Racers > UI Toolkit > Regerar Fontes`. O instalador cria
`UnityEngine.TextCore.Text.FontAsset`, que é o tipo aceito pelo UI Toolkit.
`TMP_FontAsset` é rejeitado pelo importador USS e faz o texto desaparecer.

O comando usa, para cada fonte:
